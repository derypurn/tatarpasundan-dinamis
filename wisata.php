  ...
  <div class="container">
    ...
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        echo '<div class="card">';
        echo '<img src="wisatafoto/' . $row["foto"] . '" alt="Foto ' . $row["nama_wisata"] . '" style="width:100%;">';
        echo '<h2>' . $row["nama_wisata"] . '</h2>';
        echo '<p>' . $row["Deskripsi"] . '</p>';
        echo '<a href="' . $row["link"] . '" target="_blank"><button class="tonton-video">Lihat  Maps</button></a>';
        echo '</div>';
      }
    }
    ?>
